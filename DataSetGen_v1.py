...
class dataset:
    ...
    def generate_ds2(self, size=10000):
        ...
        df = pd.DataFrame(minmax_scale(df, axis=0, feature_range=(0, 1)), columns=features)
        df = df.astype(float)
        df = df.assign(y=df.x12.apply(lambda x: 1 if (x >= 0.5) else 0))
        print(df.corr())
    ...
    def generate_ds3(self, size=10000):
        ...
        df = pd.DataFrame(minmax_scale(df, axis=0, feature_range=(0, 1)), columns=features)
        df = df.astype(float)
        df = df.assign(y=df[["x11", "x12"]].apply(lambda x: 1 if (x[0] >= 0.5 and x[1] >= 0.5) else 0, axis=1))
        print(df.corr())
    ...
    def generate_ds6(self, size=10000):
        ...
        df = pd.DataFrame(minmax_scale(df, axis=0, feature_range=(0, 1)), columns=features)
        df = df.astype(float)
        df = df.assign(y=df[["x4", "x5"]].apply(lambda x: 1 if (x[0] >= 0.5 or x[1] == 1) else 0, axis=1))
        print(df.corr())
    ...
    def generate_ds9(self, size=10000):
        ...
        df = pd.DataFrame(minmax_scale(df, axis=0, feature_range=(0, 1)), columns=features)
        df = df.astype(float)
        df = df.assign(y=df[["x2", "x3", "x4", "x5"]].apply(lambda x: 1 if ( x[0]*5 > 0.5 or x[1]*2 > 0.5 or (x[2] >= 0.5 and x[3] == 1) )else 0, axis=1))
        print(df.corr())
    ...
    def generate_ds10(self, size=10000):
        ...
        df = pd.DataFrame(minmax_scale(df, axis=0, feature_range=(0, 1)), columns=features)
        df = df.astype(float)
        df = df.assign(y=df[["x1", "x2", "x3", "x4", "x5"]].apply(lambda x: 1 if (x[0]*5 > 0.5 or  x[1]*3 > 0.5 or x[2]*2 > 0.5 or (x[3] >= 0.5 and x[4] == 1) )else 0, axis=1))
        print(df.corr())
    ...
    def generate_ds11(self, size=10000):
        ...
        df = pd.DataFrame(minmax_scale(df, axis=0, feature_range=(0, 1)), columns=features)
        df = df.astype(float)
        df = df.assign(y=df[["x1", "x2", "x3", "x4", "x5"]].apply(lambda x: 1 if (x[0] * 5) + (x[1] * 4) + (x[2] * 3) + (x[3] * 2) + x[4] > 0.5 else 0, axis=1))
        print(df.corr())
